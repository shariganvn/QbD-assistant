...
export function validateGateEvidence(evidence, gateId) {
  ...
  if (!evidence.assertions_summary || summaryKeys.some((key) => !Number.isSafeInteger(evidence.assertions_summary[key]) || evidence.assertions_summary[key] < 0)) errors.push("assertions summary is invalid");
  else if (evidence.assertions_summary.total !== evidence.assertions_summary.passed + evidence.assertions_summary.failed + evidence.assertions_summary.skipped + evidence.assertions_summary.todo + evidence.assertions_summary.cancelled) errors.push("assertions summary is inconsistent");
