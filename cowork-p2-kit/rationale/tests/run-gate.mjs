...
if (!/^G-RL-0[1-5]$/.test(gateId ?? "") || testPaths.length === 0) {
  process.stderr.write("Usage: run-gate.mjs G-RL-0[1-5] <test-path> [test-path ...]\n");
  process.exit(1);
}
...
const evidence = {
  ...
  run_id: randomUUID(),
  suite_run_id: process.env.RATIONALE_SUITE_RUN_ID ?? randomUUID(),
  assertions_summary: { total: summary.tests, passed: summary.pass, failed: summary.fail, skipped: summary.skipped, todo: summary.todo, cancelled: summary.cancelled },
  timeout_ms: 300000,
...
const evidenceDir = resolve(process.env.RATIONALE_GATE_EVIDENCE_DIR ?? "docs/reports/qbd-rationale-report-layer/gates");
