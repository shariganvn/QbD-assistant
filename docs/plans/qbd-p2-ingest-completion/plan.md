...
`completed` requires all listed gates to be `pass` with readable evidence.
...
4. Step 4 runs after production behavior is complete; it cannot weaken a gate to accommodate code.
...
- Missing/unsupported dependency capability is reported explicitly and never treated as PASS.
...
- `is-complex` is absent from the installed repo-local LiteParse CLI. The implementation must expose
  `unsupported`, not fabricate an empty eligibility result.
